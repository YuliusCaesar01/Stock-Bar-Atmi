<x-app-layout>
    ...
    @foreach ($satuan as $s)
        <!-- Edit Modal -->
        <div id="editSatuanModal-{{ $s->id }}" tabindex="-1" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 w-full md:inset-0 h-modal md:h-full">
            <div class="relative p-4 w-full max-w-md h-full md:h-auto">
                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                    ...
                    <div class="py-6 px-6 lg:px-8">
                        <h3 class="mb-4 text-xl font-medium text-gray-900 dark:text-white">Edit Nama Satuan</h3>
                        <form class="space-y-6" action="{{ route('setupsatuan.update', $s->id) }}" method="POST">
                            @csrf
                            @method('PUT')
                            <div>
                                <label for="kd_satuan" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Kode Satuan</label>
                                <input type="text" name="kd_satuan" id="kd_satuan" value="{{ $s->kd_satuan }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white" required>
                            </div>
                            <div>
                                <label for="nama_satuan" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Nama Satuan</label>
                                <input type="text" name="nama_satuan" id="nama_satuan" value="{{ $s->nama_satuan }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white" required>
                            </div>
                            <button type="submit" class="w-full text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Submit</button>
                        </form>
                    </div>
