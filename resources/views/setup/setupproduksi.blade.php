<x-app-layout>
                            ...
                            @foreach ($produksi as $s)
                                <tr>
                                    ...
                                    <td class="px-6 py-4">
                                        ...
                                        <!-- Delete Button -->
                                        <form id="delete-form-{{ $s->id }}" action="{{ route('setupproduksi.destroy', $s->id) }}" method="POST" class="inline-block">
                                            @csrf
                                            @method('DELETE')
    ...
    @foreach ($produksi as $s)
        <!-- Edit Modal -->
        <div id="editProduksiModal-{{ $s->id }}" tabindex="-1" class="hidden overflow-y-auto overflow-x-hidden fixed inset-0 z-50 flex items-center justify-center h-screen">
            <div class="relative p-4 w-full max-w-md h-full md:h-auto">
                <div class="relative bg-white rounded-lg shadow dark:bg-gray-400 border-4 border-gray-800">
                    <button type="button" class="absolute top-3 right-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center" data-modal-hide>
                        <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <div class="py-6 px-6 lg:px-8">
                        <h3 class="mb-4 text-xl font-medium text-gray-900 dark:text-dark">Edit Nama Produksi</h3>
                        <form class="space-y-6" action="{{ route('setupproduksi.update', $s->id) }}" method="POST">
                            ...
                            <div>
                                <label for="nama_prod" class="block mb-2 text-sm font-medium text-gray-900 dark:text-dark">Nama Prod</label>
                                <input type="text" name="nama_prod" id="nama_prod" value="{{ $s->nama_prod }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-100 dark:border-gray-500 dark:placeholder-gray-400 dark:text-dark" required>
                            </div>
                            <div>
                                <label for="keterangan" class="block mb-2 text-sm font-medium text-gray-900 dark:text-dark">Keterangan</label>
