<x-app-layout>
                        ...
                        <tbody>
                            @foreach ($masterakuns as $s)
                                <tr>
                                    ...
                                    <td class="px-6 py-4">{{ $s->jenis_barang }}</td>
                                    <td class="px-6 py-4">
                                        <!-- Edit Button -->
                                        ...
                                        <!-- Delete Button -->
                                        <form id="delete-form-{{ $s->id }}" action="{{ route('setupmasterakun.destroy', $s->id) }}" method="POST" class="inline-block">
                                            @csrf
                                            @method('DELETE')
            ...
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-400 border-4 border-gray-800">
                ...
                <div class="py-6 px-6 lg:px-8">
                    ...
                    <form class="space-y-6" action="{{ route('setupmasterakun.store') }}" method="POST">
                        ...
                        <div>
                            <label for="jenis_barang" class="block mb-2 text-sm font-medium text-gray-900 dark:text-dark">Jenis Barang</label>
                            <input type="text" name="jenis_barang" id="jenis_barang" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-100 dark:border-gray-500 dark:placeholder-gray-400 dark:text-dark" required>
                        </div>
    ...
    @foreach ($masterakuns as $s)
        <!-- Edit Modal -->
        <div id="editMasterakunModal-{{ $s->id }}" tabindex="-1" class="hidden overflow-y-auto overflow-x-hidden fixed inset-0 z-50 flex items-center justify-center h-screen">
            <div class="relative p-4 w-full max-w-md h-full md:h-auto">
                <div class="relative bg-white rounded-lg shadow dark:bg-gray-400 border-4 border-gray-800">
                    <button type="button" class="absolute top-3 right-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center" data-modal-hide>
                        <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <div class="py-6 px-6 lg:px-8">
                        <h3 class="mb-4 text-xl font-medium text-gray-900 dark:text-dark">Edit Nama Akun</h3>
                        <form class="space-y-6" action="{{ route('setupmasterakun.update', $s->id) }}" method="POST">
                            @csrf
                            @method('PUT')
                            <div>
                                <label for="kd_akun" class="block mb-2 text-sm font-medium text-gray-900 dark:text-dark">Kode Akun</label>
                                <input type="text" name="kd_akun" id="kd_akun" value="{{ $s->kd_akun }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-100 dark:border-gray-500 dark:placeholder-gray-400 dark:text-dark" required>
                            </div>
                            <div>
                                <label for="nama_akun" class="block mb-2 text-sm font-medium text-gray-900 dark:text-dark">Nama Akun</label>
                                <input type="text" name="nama_akun" id="nama_akun" value="{{ $s->nama_akun }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-100 dark:border-gray-500 dark:placeholder-gray-400 dark:text-dark" required>
                            </div>
                            <div>
                                <label for="kelompok" class="block mb-2 text-sm font-medium text-gray-900 dark:text-dark">Kelompok</label>
                                <input type="text" name="kelompok" id="kelompok" value="{{ $s->kelompok }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-100 dark:border-gray-500 dark:placeholder-gray-400 dark:text-dark" required>
                            </div>
                            <div>
                                <label for="jenis_barang" class="block mb-2 text-sm font-medium text-gray-900 dark:text-dark">Jenis Barang</label>
                                <input type="text" name="jenis_barang" id="jenis_barang" value="{{ $s->jenis_barang }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-100 dark:border-gray-500 dark:placeholder-gray-400 dark:text-dark" required>
                            </div>
                            <button type="submit" class="w-full text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Submit</button>
                        </form>
                    </div>
